...
static int nextrune(void)
{
	...
	if (g.yychar == '\\') {
		g.source += chartorune(&g.yychar, g.source);
		switch (g.yychar) {
		case 0: die(_("unterminated escape sequence")); break;
		...
		if (isunicodeletter(g.yychar) || g.yychar == '_') /* check identity escape */
			die(_("invalid escape character"));
		return 0;
	}
	return 0;
}
...
static void newcclass(void)
{
	if (g.ncclass >= nelem(g.prog->cclass))
		die(_("too many character classes"));
...
{
	if (a > b)
		die(_("invalid character class range"));
	if (g.yycc->end + 2 == g.yycc->spans + nelem(g.yycc->spans))
		die(_("too many character class ranges"));
	*g.yycc->end++ = a;
	*g.yycc->end++ = b;
}
...
static int lexclass(void)
{
	...
	for (;;) {
		if (g.yychar == 0)
			die(_("unterminated character class"));
		if (!quoted && g.yychar == ']')
			break;
...
static Renode *newrep(Renode *atom, int ng, int min, int max)
{
	...
	if (max == REPINF && empty(atom))
		die(_("infinite loop matching the empty string"));
	rep->ng = ng;
	rep->m = min;
	rep->n = max;
	rep->x = atom;
...
static Renode *parserep(void)
{
	...
	if (accept_char('^')) return newnode(P_BOL);
	if (accept_char('$')) return newnode(P_EOL);
	if (accept_char(L_WORD)) return newnode(P_WORD);
	if (accept_char(L_NWORD)) return newnode(P_NWORD);
	...
	if (g.lookahead == L_COUNT) {
		...
		if (max < min)
			die(_("invalid quantifier"));
		return newrep(atom, accept_char('?'), min, max);
	}
	if (accept_char('*')) return newrep(atom, accept_char('?'), 0, REPINF);
	if (accept_char('+')) return newrep(atom, accept_char('?'), 1, REPINF);
	if (accept_char('?')) return newrep(atom, accept_char('?'), 0, 1);
	return atom;
}
...
static unsigned int count(Renode *node)
{
	...
	case P_REP:
		...
		if (n > MAXPROG) die(_("program too large"));
		return n;
...
Reprog *regexp_comp(const char *pattern, int cflags, const char **errorp)
{
	...
	if (!g.prog)
		die(_("cannot allocate regular expression"));
	n = strlen(pattern) * 2;
	if (n > 0) {
		g.pstart = g.pend = malloc(sizeof (Renode) * n);
		if (!g.pstart)
			die(_("cannot allocate regular expression parse list"));
	}
